...
Drupal.behaviors.wh_splash_form = function(context) {
  // Override click handling of fake form submit.
  // TODO: Consider hijacking form submit instead,
  //   so that it works in all browsers whether clicked
  //   or enter pressed.
  var hidden_form_container;

  // Consider using a more specific class or id,
  // anticipating future splash pages with
  // potentially different forms.
  $('#splash-form input:image')
  .click(function(e) {
    e.preventDefault();

    splash_form = $("#wh-splash-"+to_show+" #splash-form form").attr("id");
    if (splash_form === "") { var splash_form = "get-email-updates"; }
    // Don't do it if fake val is still "Email Address"
    var fake_email = $('#wh-splash-'+to_show+' #splash-form input.splash-form-input:text').val();
    if (!(hidden_form_container || (fake_email == ''))) {
      hidden_form_container = $('<div></div>');
      hidden_form_container
      .css({'display': 'none'})
      .prependTo('#wh-splash-wrapper')
      .load('/splash/'+splash_form+'/modal', function() {
        $('#edit-submitted-email').val(fake_email);
        $.ajax({
          type: "POST",
          url: "/"+splash_form,
          data: $("form[id^=webform-client-form-]").serialize(),
          success: function() {
          }
        });
      });
      ...
      window.scrollTo(0,0); wh_splash_close();
    }
  });
}
